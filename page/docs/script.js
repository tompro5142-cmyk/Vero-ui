...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const populatePageContent = () => {
    ...
    const creator = settings.apiSettings?.creator || "Veron Api'S"

    setPageContent(DOM.pageTitle, settings.name, "Veron Api'S")
    setPageContent(DOM.wm, `© ${currentYear} Veron Api'S Corp. All rights reversed.`)
    setPageContent(DOM.appName, settings.name, "Veron Api'S")
